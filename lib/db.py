...
class _LasyConnection(object):
    ...
    # 这些都是mysql.connector这个库的方法
    # engine是mysql.connector这个库的东东
    def commit(self):
...
_db_ctx = _DbCtx()
'''
_db_ctx是全局变量，但是因为引用的是_DbCtx对象，情况就不同了，直接说结果就是，
每个线程里的_db_ctx都是不一样的，都是自己线程的一个新对象。假设有n个线程，
那么就有n+1个不同的_DbCtx对象，虽然他们都是通过_db_ctx引用。

这样的好处就是，
_DbCtx里有connect对象也都是各个线程独立的了呀（通过_LasyConnection()函数获得的），每个线程有自己的连接。
实现这个功能关键在于_DbCtx是继承自包threading里的local类
'''

# global engine object:
# engine是一个mysql.connector的东东
engine = None
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    # 将数据写入params，如果有些选填数据则代替默认，如果没有则默认
